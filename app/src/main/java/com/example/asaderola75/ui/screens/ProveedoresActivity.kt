...
class ProveedoresActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel.proveedores.observe(this) { lista ->
            ...
            rv.adapter = ProveedorAdapter(
                lista,
                onEditar = { proveedor -> mostrarDialogoEditar(proveedor) },
                onEliminar = { proveedor -> confirmarEliminar(proveedor) },
                onCambiarEstado = { proveedor -> viewModel.toggleEstado(token, proveedor.id_proveedor) }
            )
        }
    ...
    private fun mostrarDialogoCrear() {
        val view = LayoutInflater.from(this).inflate(R.layout.dialog_proveedor, null)
    ...
    private fun mostrarDialogoEditar(proveedor: Proveedor) {
        ...
        AlertDialog.Builder(this, R.style.DialogOscuro)
            ...
            .setPositiveButton("Guardar") { _, _ ->
                ...
                viewModel.updateProveedor(token, proveedor.id_proveedor, body)
            }
            .setNegativeButton("Cancelar", null)
            .show()
    }
    ...
    private fun confirmarEliminar(proveedor: Proveedor) {
        AlertDialog.Builder(this, R.style.DialogOscuro)
            ...
            .setPositiveButton("Eliminar") { _, _ ->
                viewModel.deleteProveedor(token, proveedor.id_proveedor)
            }
            .setNegativeButton("Cancelar", null)
            .show()
    }
